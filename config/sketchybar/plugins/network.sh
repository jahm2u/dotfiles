...
# Update the network item with smart display
# Use $NAME if set, otherwise default to 'network'
ITEM_NAME="${NAME:-network}"

# Smart display - show full label only on issues or disconnection
if [ "$COLOR" = "$RED" ] || [ "$LABEL" = "Disconnected" ]; then
    # Show both icon and label when there's an issue
    sketchybar --set "$ITEM_NAME" icon="$ICON" label="$LABEL" label.drawing=on
else
    # Show only icon when connected and working
