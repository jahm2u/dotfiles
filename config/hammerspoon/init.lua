...
-- Function to set device-specific AeroSpace gaps and Sketchybar display settings
function setDeviceSpecificGaps()
    -- Detect device type and display setup
    local task = hs.task.new("/usr/bin/pmset", function(exitCode, stdOut, stdErr)
        local isLaptop = false
        if exitCode == 0 and stdOut then
            isLaptop = stdOut:find("InternalBattery") ~= nil or stdOut:find("Battery") ~= nil
        end

        -- Check if built-in display (laptop screen) is the primary display
        local builtInScreen = nil
        local externalScreens = {}

        print("=== DEBUG: Screen Detection ===")
        for _, screen in pairs(hs.screen.allScreens()) do
            local screenName = screen:name()
            local isPrimary = screen == hs.screen.primaryScreen()
            print(string.format("Screen: '%s', Primary: %s", screenName, isPrimary))

            -- Built-in display typically has "Built-in" in name or is the laptop's internal display
            if screenName:find("Built%-in") or screenName:find("Liquid Retina") or screenName:find("MacBook") then
                builtInScreen = screen
                print("  -> Detected as built-in screen")
            else
                table.insert(externalScreens, screen)
                print("  -> Detected as external screen")
            end
        end
        print(string.format("Built-in screen found: %s", builtInScreen and "yes" or "no"))
        print(string.format("External screens count: %d", #externalScreens))

        -- Determine gap strategy:
        -- - Laptop hardware (has battery): always use 40px gap for proper spacing
        -- - Pure desktop (no laptop): use minimal 10px gap
        local laptopIsPrimary = builtInScreen and builtInScreen == hs.screen.primaryScreen()
        local hasExternalMonitor = #externalScreens > 0

        local topGap
        if isLaptop then
            if laptopIsPrimary or not hasExternalMonitor then
                topGap = "40"  -- Laptop screen needs notch clearance
                print("Laptop screen is primary - using notch clearance (40px)")
            else
                topGap = "40"  -- External monitor is primary, but still laptop hardware - needs gap
                print("External monitor is primary on laptop - using 40px gap")
            end
        else
            topGap = "10"  -- Pure desktop setup (no laptop involved)
            print("Pure desktop setup - using minimal gap (10px)")
        end

        -- Get monitor information for per-monitor configuration
        local primaryScreen = hs.screen.primaryScreen()
        local primaryName = primaryScreen:name()

        -- Create per-monitor gap configuration
        local gapConfig
        print(string.format("=== DEBUG: Gap Configuration ==="))
        print(string.format("hasExternalMonitor: %s, builtInScreen exists: %s", hasExternalMonitor, builtInScreen and "yes" or "no"))

        if hasExternalMonitor and builtInScreen then
            -- Multi-monitor setup: different gaps per display
            local builtInName = builtInScreen:name()
            gapConfig = string.format('outer.top = [{ monitor."%s" = 40 }, { monitor."%s" = 10 }, 40]', primaryName, builtInName)
            print(string.format("Multi-monitor setup: %s=40px, %s=10px", primaryName, builtInName))
        else
            -- Single monitor setup
            gapConfig = string.format("outer.top = %s", topGap)
            print(string.format("Single monitor setup: %spx", topGap))
        end

        print(string.format("Generated gap config: %s", gapConfig))

        -- Update AeroSpace config with per-monitor gaps
        local sedCommand = string.format("sed -i '' 's/outer\\.top = .*/    %s/' ~/.config/aerospace/aerospace.toml", gapConfig:gsub("/", "\\/"))
        print(string.format("Sed command: %s", sedCommand))
        local updateTask = hs.task.new("/bin/bash", function(exitCode, stdOut, stdErr)
            if exitCode == 0 then
                -- Reload AeroSpace config
                hs.task.new("/opt/homebrew/bin/aerospace", nil, {"reload-config"}):start()
                print("Updated AeroSpace per-monitor gaps")

                -- Also restart Sketchybar to apply display changes
                hs.task.new("/opt/homebrew/bin/brew", nil, {"services", "restart", "sketchybar"}):start()
            else
                print("Failed to update AeroSpace gaps: " .. (stdErr or "unknown error"))
            end
        end, {"-c", sedCommand})
        updateTask:start()
    end, {"-g", "batt"})
...
-- Set gaps on startup and when configuration changes
setDeviceSpecificGaps()

-- Add temporary debug hotkey
hs.hotkey.bind({"ctrl", "alt", "cmd"}, "d", function()
    print("=== MANUAL DEBUG TRIGGER ===")
    setDeviceSpecificGaps()
end)

-- Also trigger when screen configuration changes
hs.screen.watcher.new(function()
    print("Screen configuration changed, adjusting gaps...")
    hs.timer.doAfter(1, setDeviceSpecificGaps) -- Small delay to let system settle
end):start()
